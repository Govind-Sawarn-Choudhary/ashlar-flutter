...
/// User OTP verification — same layout as [LawyerOtpScreen], 6-digit OTP.
...
class _UserOtpScreenState extends State<UserOtpScreen> {
  ...
  List<double> get _otpCellXInGroup {
    final gap =
        (_otpGroupWidth - _otpLength * _otpCellSize) / (_otpLength - 1);
  ...
  Future<void> _verifyOtp() async {
    ...
    try {
      final response = await UserAuthRepository.instance.verifyOtp(
      ...
      final route = UserAuthRepository.instance.routeForNextStep(
        response.nextRoute,
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('OTP verification failed. Check backend connection.'),
        ),
      );
    } finally {
      if (mounted) {
  ...
  Future<void> _onResendTap() async {
    ...
    try {
      await UserAuthRepository.instance.sendOtp(widget.phoneNumber);
      if (!mounted) {
        return;
      }
      _startResendTimer();
